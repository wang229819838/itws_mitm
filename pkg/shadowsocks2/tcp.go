...
func tcpRemote(addr string, shadow func(net.Conn) net.Conn) {
	...
	for {
		...
		go func() {
			...
			// 	logf("relay error: %v", err)
			// }
			if err = relay2(sc, rc,nil); err != nil {
				logf("relay error: %v", err)
			}
		}()
	}
...
func relay2(clientConn, mitmConn, targetConn net.Conn) error {
	// Set up channels for error handling
	errChan := make(chan error, 2)

	// Relay data from client to MITM
	go func() {
		_, err := io.Copy(mitmConn, clientConn)
		errChan <- err
	}()

	// Relay data from MITM to target
	go func() {
		_, err := io.Copy(targetConn, mitmConn)
		errChan <- err
	}()

	// Relay data from target to MITM
	go func() {
		_, err := io.Copy(mitmConn, targetConn)
		errChan <- err
	}()

	// Relay data from MITM to client
	go func() {
		_, err := io.Copy(clientConn, mitmConn)
		errChan <- err
	}()
	...
	for i := 0; i < 4; i++ {
		if err := <-errChan; err != nil {
			// Handle error, close connections
			clientConn.Close()
			mitmConn.Close()
			targetConn.Close()
			return err
